...
def get_taboo_moves(game_state):
    ...
    selected_items = [tuple([item.i, item.j]) for item in all_moves]  # Fill a list with only the coordinates, not the value
...
def check_sub_square_values(board: SudokuBoard, x, y):
    ...
    (p, q) = (int(math.ceil((x + 1) / board.m) * board.m) - 1,
              int(math.ceil((y + 1) / board.n) * board.n) - 1)  # calculates the highest coordinates in the sub-square
...
def game_result(game_state: GameState, our_player_number):
    ...
    @Return: Returns 1 if we win on this board and 2 if we lose.
    """
...
class MonteCarloTreeSearchNode:
    ...
    def best_action(self):
        """
        This function first selects a starting move to perform rollout on.
